...
const COLLABORATOR = 'd7j9tts2olvgic6776g0y766ekedfq6s1.oast.live';
...
const LABS = [
  ...
  { topic: 'sql-injection', num: 17, dir: 'SQLInjection', script: 'exploit-lab17.py', title: 'Blind SQL injection with OOB data exfiltration', timeout: 300000, needsCollab: true },
  { topic: 'cross-site-scripting', num: 22, dir: 'XSS', script: 'exploit-lab22.py', title: 'Exploiting XSS to steal cookies', timeout: 300000, needsCollab: true },
  { topic: 'cross-site-scripting', num: 23, dir: 'XSS', script: 'exploit-lab23.py', title: 'Exploiting XSS to capture passwords', timeout: 300000, needsCollab: true },
  { topic: 'cross-site-scripting', num: 29, dir: 'XSS', script: 'exploit-lab29.py', title: 'Reflected XSS with strict CSP', timeout: 300000 },
  { topic: 'ssrf', num: 6, dir: 'SSRF', script: 'exploit-lab06.py', title: 'Blind SSRF with Shellshock', timeout: 300000, needsCollab: true },
  { topic: 'os-command-injection', num: 4, dir: 'OSCommandInjection', script: 'exploit-lab04.py', title: 'Blind OS command injection OOB interaction', timeout: 300000, needsCollab: true },
  { topic: 'os-command-injection', num: 5, dir: 'OSCommandInjection', script: 'exploit-lab05.py', title: 'Blind OS command injection OOB data exfiltration', timeout: 300000, needsCollab: true },
...
async function solveLab(browser, lab) {
  ...
  try {
    ...
    try {
      const cmd = lab.needsCollab
        ? `python3 "${solverPath}" -U "${base}" -C "${COLLABORATOR}"`
...
async function main() {
  console.log('\nBatch B Fixed: SQLi, XSS, SSRF, OSCI, WebSockets\n');
  ...
  console.log(`\n========================================`);
  console.log(`Batch B Fixed Result: ${solved} solved, ${failed} failed, ${skipped} skipped`);
  console.log(`========================================\n`);
  await browser.close();
}
